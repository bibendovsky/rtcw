...
#if !defined RTCW_ET
	newShader = static_cast<shader_t*> (R_CacheShaderAlloc( sizeof( shader_t ) ));
#else
	newShader = static_cast<shader_t*> (R_CacheShaderAlloc( shader.lightmapIndex < 0 ? va( "%s lm: %i", shader.name, shader.lightmapIndex ) : NULL, sizeof( shader_t ) ));
#endif // RTCW_XX
...
#if !defined RTCW_ET
		newShader->stages[i] = static_cast<shaderStage_t*> (R_CacheShaderAlloc( sizeof( stages[i] ) ));
#else
		newShader->stages[i] = static_cast<shaderStage_t*> (R_CacheShaderAlloc( NULL, sizeof( stages[i] ) ));
#endif // RTCW_XX
...
#if !defined RTCW_ET
			newShader->stages[i]->bundle[b].texMods = static_cast<texModInfo_t*> (R_CacheShaderAlloc( size ));
#else
			newShader->stages[i]->bundle[b].texMods = static_cast<texModInfo_t*> (R_CacheShaderAlloc( NULL, size ));
#endif // RTCW_XX
...
qboolean RE_LoadDynamicShader( const char *shadername, const char *shadertext ) {
	...
	dptr->shadertext = static_cast<char*> (Z_Malloc( strlen( shadertext ) + 1 ));
	if ( !dptr->shadertext ) {
...
static void ScanAndLoadShaderFiles( void ) {
	...
	s_shaderText = static_cast<char*> (ri.Hunk_Alloc( sum + numShaders * 2, h_low ));
