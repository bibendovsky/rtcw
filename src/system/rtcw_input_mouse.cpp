...
void Mouse::handle_event(const SDL_Event& e)
{
    ...
    default:
        assert(!"Expected mouse event.");
        return;
    }
}

void Mouse::activate(bool value)
{
    if (!is_initialized_)
        return;
...
// (static)
int Mouse::get_game_button_by_sys_button(int sys_button)
{
    switch (sys_button) {
...
// (static)
int Mouse::get_state_index_by_sys_button(int sys_button)
{
    switch (sys_button) {
    case SDL_BUTTON_LEFT:
        return 0;
...
// (static)
int Mouse::get_game_button_by_state_index(int state_index)
{
    switch (state_index) {
    case 0:
        return K_MOUSE1;
