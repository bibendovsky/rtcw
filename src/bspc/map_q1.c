...
// Tab Size:		3
// NOTES:			the recursive splitting of the huge brush sometimes still
//						creates bad brushes
//===========================================================================
...
int Q1_SolidTree_r( int nodenum ) {
	...
	if ( !Q1_SolidTree_r( q1_dnodes[nodenum].children[1] ) ) {
		return false;
	}
	return true;
...
bspbrush_t *Q1_CreateBrushes_r( bspbrush_t *brush, int nodenum ) {
	...
	if ( back ) {
		back = Q1_CreateBrushes_r( back, q1_dnodes[nodenum].children[1] );
	}
	...
	for ( brush = brushlist; brush; brush = nextbrush )
	{
		...
		for ( sn = 0; sn < brush->numsides; sn++ )
		{
			...
			if ( bestfacenum >= 0 ) {
				...
				miptex = ( q1_miptex_t * )( (byte *)miptexlump + miptexlump->dataofs[q1_texinfo[texinfonum].miptex] );
