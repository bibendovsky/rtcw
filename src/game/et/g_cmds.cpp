	...
	if ( Q_stricmpn( name, "skill", 5 ) == 0 ) {
		if ( hasAmount ) {
			if ( amount >= 0 && amount < SK_NUM_SKILLS ) {
				G_AddSkillPoints( ent, skillType_t (amount), 20 );
				G_DebugAddSkillPoints( ent, skillType_t (amount), 20, "give skill" );
			}
		} else {
			// bumps all skills with 1 level
			for ( i = 0; i < SK_NUM_SKILLS; i++ ) {
				G_AddSkillPoints( ent, skillType_t (i), 20 );
				G_DebugAddSkillPoints( ent, skillType_t (i), 20, "give skill" );
			}
		}
		return;
	}
