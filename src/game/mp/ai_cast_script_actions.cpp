			...
			} else
			{
				...
				if ( !token[0] || !Q_stricmp( token,"nostop" ) ) {
					...
					if ( !token[0] || Q_stricmp( token,"noattack" ) ) {
						...
						if ( fire ) {
							for ( i = 0; i < 2; i++ ) {
								diff = c::abs( AngleDifference( cs->bs->viewangles[i], cs->bs->ideal_viewangles[i] ) );
								if ( diff < 20 ) {
									// force fire
...
qboolean AICast_ScriptAction_PlayAnim( cast_state_t *cs, const char* params ) {
	...
	if ( level.animScriptData.modelInfo[level.animScriptData.clientModels[cs->entityNum] - 1].version > 1 ) { // new (scripted) model
		...
		if ( cs->castScriptStatus.scriptFlags & SFL_FIRST_CALL ) {
			...
			BG_PlayAnimName( &( client->ps ), tokens[0], animBodyPart_t (BG_IndexForString( tokens[1], animBodyPartsStr, qfalse )), qtrue, qfalse, qtrue );
...
qboolean AICast_ScriptAction_FireAtTarget( cast_state_t *cs, const char* params ) {
	...
	vectoangles( vec, cs->bs->ideal_viewangles );
	for ( i = 0; i < 2; i++ ) {
		diff = c::abs( AngleDifference( cs->bs->cur_ps.viewangles[i], cs->bs->ideal_viewangles[i] ) );
...
qboolean AICast_ScriptAction_Mount( cast_state_t *cs, const char* params ) {
	...
	if ( c::fabs( cs->bs->ideal_viewangles[YAW] - cs->bs->viewangles[YAW] ) < 10 ) {
