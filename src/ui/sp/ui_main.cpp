...
static void WM_ChangePlayerType() {
	...
	for ( i = 0; i < 4; i++ ) {
		...
		if ( i == playerType ) {
			Menu_ShowItemByName( static_cast<menuDef_t*> (itemdef->parent), playerTypes[i].name, qtrue );
		} else {
			Menu_ShowItemByName( static_cast<menuDef_t*> (itemdef->parent), playerTypes[i].name, qfalse );
		}
...
static void UI_StartServerRefresh( qboolean full ) {
	...
	if ( ui_netSource.integer == AS_GLOBAL || ui_netSource.integer == AS_MPLAYER ) {
		...
		if ( strlen( ptr ) ) {
			...
			trap_Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %d full empty\n", i, static_cast<int> (trap_Cvar_VariableValue( "protocol" )) ) );
		}
	}
}
// -NERVE - SMF
