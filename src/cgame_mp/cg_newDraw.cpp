...
//		case WP_DYNAMITE: // JPW NERVE these recharge
	...
	// ammo
	ammovalue = cg.snap->ps.ammo[BG_FindAmmoForWeapon( weapon_t (weap) )];
	ammovalue2 = ps->ammoclip[BG_FindClipForWeapon( weapon_t (weap) )];

	// clip
	clipvalue = ps->ammoclip[BG_FindClipForWeapon( weapon_t (weap) )];
	if ( special ) {
