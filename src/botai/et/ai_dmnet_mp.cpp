...
//	}
	...
	if ( bs->lastClassCheck < level.time - 4000 ) {   // check for a better class
		bs->mpClass = BotSuggestClass( bs, team_t (bs->mpTeam) );
...
int AINode_MP_GiveAmmo( bot_state_t *bs ) {
	...
	// reload?
	if ( ( bs->last_fire != level.time ) && ( bs->cur_ps.ammoclip[BG_FindClipForWeapon( weapon_t (bs->cur_ps.weapon) )]
											  < (int)( 0.8 * ( GetAmmoTableData( bs->cur_ps.weapon ) )->maxclip ) )
		 && bs->cur_ps.ammo[BG_FindAmmoForWeapon( weapon_t (bs->cur_ps.weapon) )] ) {
...
int AINode_MP_MedicGiveHealth( bot_state_t *bs ) {
	...
	// reload?
	if ( ( bs->last_fire != level.time ) && ( bs->cur_ps.ammoclip[BG_FindClipForWeapon( weapon_t (bs->cur_ps.weapon) )] < (int)( 0.8 * GetAmmoTableData( bs->cur_ps.weapon )->maxclip ) ) && bs->cur_ps.ammo[BG_FindAmmoForWeapon( weapon_t (bs->cur_ps.weapon) )] ) {
		trap_EA_Reload( bs->client );
	}
	//
	return qtrue;
...
int AINode_MP_FixMG42( bot_state_t *bs ) {
	...
	} else if ( ( bs->last_fire != level.time ) && ( bs->cur_ps.ammoclip[BG_FindClipForWeapon( weapon_t (bs->cur_ps.weapon) )] < (int)( 0.8 * GetAmmoTableData( bs->cur_ps.weapon )->maxclip ) ) && bs->cur_ps.ammo[BG_FindAmmoForWeapon( weapon_t (bs->cur_ps.weapon) )] ) { // reload?
		trap_EA_Reload( bs->client );
	}
...
int AINode_MP_SniperSpot( bot_state_t *bs ) {
	...
	// reload?
	if ( ( level.time - bs->last_fire > 2000 ) && ( bs->cur_ps.ammoclip[BG_FindClipForWeapon( weapon_t (bs->cur_ps.weapon) )] < (int)( 0.8 * GetAmmoTableData( bs->cur_ps.weapon )->maxclip ) ) && bs->cur_ps.ammo[BG_FindAmmoForWeapon( weapon_t (bs->cur_ps.weapon) )] ) {
		trap_EA_Reload( bs->client );
	}
