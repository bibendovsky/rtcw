...
void Bot_ScriptParseAllCharacters() {
	...
	while ( 1 )
	{
		...
		// allocate the name
		bsd->name = static_cast<char*> (G_Alloc( strlen( token ) + 1 ));
		...
		// allocate the params
		bsd->params = static_cast<char*> (G_Alloc( strlen( params ) + 1 ));
		Q_strncpyz( bsd->params, params, strlen( params ) + 1 );
		// allocate memory for this character script
		bsd->data = static_cast<bot_script_data_t*> (G_Alloc( sizeof( bot_script_data_t ) ));
