...
//----(SA)	added translation lookup
	Q_strncpyz( reinterpret_cast<char*> (cg.centerPrint), CG_translateString( (char*)str ), sizeof( cg.centerPrint ) );
//----(SA)	end
	...
	while ( *s ) {
		...
		if ( !Q_strncmp( reinterpret_cast<const char*> (s), "\\n", 1 ) ) {
			cg.centerPrintLines++;
			s++;
		}
		s++;
...
static void CG_DrawCenterString( void ) {
	...
	start = reinterpret_cast<char*> (cg.centerPrint);
