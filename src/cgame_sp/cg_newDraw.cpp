...
const char *CG_GetKillerText() {
	...
	if ( cg.killerName[0] ) {
		s = reinterpret_cast<const unsigned char*> (va( "Fragged by %s", cg.killerName ));
	}
...
const char *CG_GetGameStatusText() {
	...
	if ( cgs.gametype < GT_TEAM ) {
		if ( cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR ) {
			s = reinterpret_cast<const unsigned char*> (va( "%s place with %i",CG_PlaceString( cg.snap->ps.persistant[PERS_RANK] + 1 ),cg.snap->ps.persistant[PERS_SCORE] ));
		}
	} else {
		if ( cg.teamScores[0] == cg.teamScores[1] ) {
			s = reinterpret_cast<const unsigned char*> (va( "Teams are tied at %i", cg.teamScores[0] ));
		} else if ( cg.teamScores[0] >= cg.teamScores[1] ) {
			s = reinterpret_cast<const unsigned char*> (va( "Red leads Blue, %i to %i", cg.teamScores[0], cg.teamScores[1] ));
		} else {
			s = reinterpret_cast<const unsigned char*> (va( "Blue leads Red, %i to %i", cg.teamScores[1], cg.teamScores[0] ));
		}
	}
	return reinterpret_cast<const char*> (s);
}
	...
	if ( text ) {
		...
		while ( s && *s && count < len ) {
			...
			if ( Q_IsColorString( s ) ) {
				...
				if ( CG_Text_Width( reinterpret_cast<const char*> (s), font, useScale, 1 ) + x > max ) {
					*maxX = 0;
					break;
				}
