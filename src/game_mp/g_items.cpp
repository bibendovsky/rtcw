...
void Add_Ammo( gentity_t *ent, int weapon, int count, qboolean fillClip ) {
	...
	if ( g_dmflags.integer & DF_NO_WEAPRELOAD ) {      // no clips
		...
		totalcount = ent->client->ps.ammo[ammoweap] + ent->client->ps.ammoclip[BG_FindClipForWeapon( weapon_t (weapon) )];
		if ( totalcount > ammoTable[ammoweap].maxammo ) {
			ent->client->ps.ammo[ammoweap] = ammoTable[ammoweap].maxammo - ent->client->ps.ammoclip[BG_FindClipForWeapon( weapon_t (weapon) )];
		}
