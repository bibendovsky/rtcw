...
*/
	...
	// reload?
	if ( ( cs->bs->cur_ps.ammoclip[cs->bs->cur_ps.weapon] < (int)( 0.75 * ammoTable[cs->bs->cur_ps.weapon].maxclip ) ) && cs->bs->cur_ps.ammo[BG_FindAmmoForWeapon( weapon_t (cs->bs->cur_ps.weapon) )] ) {
		trap_EA_Reload( cs->entityNum );
	}
...
char *AIFunc_Battle( cast_state_t *cs ) {
	...
	*/
	...
	//
	// reload?
	if ( ( cs->bs->cur_ps.ammoclip[BG_FindClipForWeapon( weapon_t (cs->bs->cur_ps.weapon) )] < (int)( ammoTable[cs->bs->cur_ps.weapon].uses ) ) ) {
		if ( AICast_GotEnoughAmmoForWeapon( cs, cs->bs->weaponnum ) ) {
			trap_EA_Reload( cs->entityNum );
		} else {    // no ammo, switch?
			AICast_ChooseWeapon( cs, qfalse );
