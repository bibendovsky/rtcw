	...
	posix_dir_()
{
	...
	{
		// Copy whole path.
		std::size_t path_size = std::strlen(path) + 1;
		if (path_size >= max_path_size)
		{
			assert(false && "Path buffer too small.");
			return;
		}
		std::memcpy(path_, path, path_size);
		// Append slash.
...
{
	SysDirPosix* const sys_dir = rtcw::mem::new_object_1<SysDirPosix>(path);
...
bool SysDirPosix::update_entry()
{
	...
	const std::size_t name_size = std::strlen(posix_dirent_->d_name) + 1;
	if (name_index_ + name_size > max_path_size)
	{
		assert(false && "Path buffer too small.");
		return false;
	}
	std::memcpy(path_ + name_index_, posix_dirent_->d_name, name_size);
	// Get file info.
...
char* Sys_Cwd()
{
	...
	char* const posix_result = getcwd(path, max_path_size);
