...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	axis = ent->count;
	VectorClear( dir );
	if ( c::fabs( other->s.origin[axis] - ent->r.absmax[axis] ) <
		 c::fabs( other->s.origin[axis] - ent->r.absmin[axis] ) ) {
...
void SP_func_door( gentity_t *ent ) {
	...
	abs_movedir[0] = c::fabs( ent->movedir[0] );
	abs_movedir[1] = c::fabs( ent->movedir[1] );
	abs_movedir[2] = c::fabs( ent->movedir[2] );
...
void Reached_Train_rotating( gentity_t *ent ) {
	...
	// Rotate the train
	frames = c::floor( float (ent->s.pos.trDuration / 100) );
...
void SP_func_pendulum( gentity_t *ent ) {
	...
	length = c::fabs( ent->r.mins[2] );
	if ( length < 8 ) {
		length = 8;
	}

	freq = 1 / ( M_PI * 2 ) * c::sqrt( g_gravity.value / ( 3 * length ) );
...
//		G_RadiusDamage(self->s.origin, self, self->damage, self->damage+40, self, MOD_EXPLOSIVE);
	...
	if ( G_GetWeaponClassForMOD( meansOfDeath_t (mod) ) >= self->constructibleStats.weaponclass ) {
		G_AddKillSkillPointsForDestruction( attacker, meansOfDeath_t (mod), &self->constructibleStats );
	}
}
