		...
		case TEAM_AXIS:
			...
			case PC_SOLDIER:
				if ( client->sess.skill[SK_HEAVY_WEAPONS] >= 4 && client->sess.playerWeapon2 == WP_MP40 ) {
					...
					client->ps.ammoclip[BG_FindClipForWeapon( weapon_t (BG_AkimboSidearm( WP_AKIMBO_LUGER )) )] = GetAmmoTableData( WP_AKIMBO_LUGER )->defaultStartingClip;
			...
			case PC_COVERTOPS:
				if ( client->sess.skill[SK_LIGHT_WEAPONS] >= 4 && ( client->sess.playerWeapon2 == WP_AKIMBO_SILENCEDLUGER || client->sess.playerWeapon2 == WP_AKIMBO_LUGER ) ) {
					client->ps.ammoclip[BG_FindClipForWeapon( weapon_t (BG_AkimboSidearm( WP_AKIMBO_SILENCEDLUGER )) )] = GetAmmoTableData( WP_AKIMBO_SILENCEDLUGER )->defaultStartingClip;
		...
		default:
			...
			case PC_SOLDIER:
				if ( client->sess.skill[SK_HEAVY_WEAPONS] >= 4 && client->sess.playerWeapon2 == WP_THOMPSON ) {
					...
					client->ps.ammoclip[BG_FindClipForWeapon( weapon_t (BG_AkimboSidearm( WP_AKIMBO_COLT )) )] = GetAmmoTableData( WP_AKIMBO_COLT )->defaultStartingClip;
			...
			case PC_COVERTOPS:
				if ( client->sess.skill[SK_LIGHT_WEAPONS] >= 4 && ( client->sess.playerWeapon2 == WP_AKIMBO_SILENCEDCOLT || client->sess.playerWeapon2 == WP_AKIMBO_COLT ) ) {
					client->ps.ammoclip[BG_FindClipForWeapon( weapon_t (BG_AkimboSidearm( WP_AKIMBO_SILENCEDCOLT )) )] = GetAmmoTableData( WP_AKIMBO_SILENCEDCOLT )->defaultStartingClip;
...
#else
		G_LogPrintf( "Forcing disconnect on active client: %" PRIiPTR "\n", ent - g_entities );
#endif // FIXME
...
//			}
	...
	if ( !revived ) {
		...
		if ( G_IsWeaponDisabled( ent, weapon_t (client->sess.latchPlayerWeapon) ) ) {
		...
		if ( G_IsWeaponDisabled( ent, weapon_t (client->sess.playerWeapon) ) ) {
			bg_playerclass_t* classInfo = BG_PlayerClassForPlayerState( &ent->client->ps );
