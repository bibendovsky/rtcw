...
void PM_CheckForReload( int weapon ) {
	...
	if (    autoreload && !( pm->ps->ammoclip[BG_FindClipForWeapon( weapon_t (weapon) )] ) &&  // clip is empty...
			pm->ps->ammo[BG_FindAmmoForWeapon( weapon_t (weapon) )] ) {          // and you have reserves
		...
		if ( pm->ps->ammo[BG_FindAmmoForWeapon( weapon_t (weapon) )] && pm->ps->ammoclip[BG_FindClipForWeapon( weapon_t (weapon) )] < ammoTable[weapon].maxclip ) {
			PM_BeginWeaponReload( weapon );
		}
...
int PM_WeaponAmmoAvailable( int wp ) {
	if ( pm->noWeapClips ) {
		...
		return pm->ps->ammoclip[BG_FindClipForWeapon( weapon_t (wp) )];
	}
}
