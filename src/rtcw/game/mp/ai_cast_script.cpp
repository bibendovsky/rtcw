...
qboolean AICast_ScriptAction_MissionSuccess( cast_state_t *cs, const char *params );
...
qboolean AICast_ScriptAction_StartCam( cast_state_t *cs, const char *params ); //----(SA)	added
qboolean AICast_ScriptAction_StartCamBlack( cast_state_t *cs, const char *params );    //----(SA)	added
...
void AICast_ScriptLoad( void ) {
	...
	level.scriptAI = static_cast<char*> (G_Alloc( len ));
...
void AICast_ScriptParse( cast_state_t *cs ) {
	...
	while ( 1 )
	{
		...
		if ( token[0] == '}' ) {
			...
			while ( ( token = COM_Parse( &pScript ) ) && ( token[0] != '}' ) )
			{
				...
				if ( strlen( params ) ) { // copy the params into the event
					curEvent->stack.items[curEvent->stack.numItems].params = static_cast<char*> (G_Alloc( strlen( params ) + 1 ));
	...
	if ( numEventItems > 0 ) {
		cs->castScriptEvents = static_cast<cast_script_event_t*> (G_Alloc( sizeof( cast_script_event_t ) * numEventItems ));
