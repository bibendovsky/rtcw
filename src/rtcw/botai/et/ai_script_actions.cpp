...
=================
*/
void Bot_ScriptError( bot_state_t *bs, const char *fmt, ... ) {
	va_list ap;
	char text[512];
	//
	va_start( ap, fmt );
...
const char *botAttributeStrings[] =
{
...
qboolean Bot_ScriptAction_SetScriptAutonomy( bot_state_t *bs, const char* params ) {
	...
	//
	level = scriptAutonomy_t (BotScriptAutonomyForString( params ));
	if ( level < 0 ) {
...
qboolean Bot_ScriptAction_SetAmmoAmount( bot_state_t *bs, const char* params ) {
	...
	if ( clipOnly ) {
		g_entities[bs->client].client->ps.ammoclip[BG_FindAmmoForWeapon( weapon_t (weapon) )] = amount;
	} else
	{
		g_entities[bs->client].client->ps.ammo[BG_FindAmmoForWeapon( weapon_t (weapon) )] = amount;
	}
