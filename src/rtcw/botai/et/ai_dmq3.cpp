...
qboolean BotGotEnoughAmmoForWeapon( bot_state_t *bs, int weapon ) {
	...
	ammo = bs->cur_ps.ammo[BG_FindAmmoForWeapon( weapon_t (weapon) )];
	clip = bs->cur_ps.ammoclip[BG_FindClipForWeapon( weapon_t (weapon) )];
...
int BotBestFightWeapon( bot_state_t *bs ) {
	...
	bestWantScale = 0.0;
	bestRange = 0.0;
	bestWeapon = weapon_t (bs->weaponnum); // default to current weapon
	bestInRange = qfalse;
...
#else
	if ( (g_entities[client].r.svFlags & SVF_BOT) == 0 ) {
#endif // FIXME
		return;
	}
