...
int Menu_ItemsMatchingGroup( menuDef_t *menu, const char *name ) {
	...
	pdest = const_cast<char*> (strstr( name, "*" )); // allow wildcard strings (ex.  "hide nb_*" would translate to "hide nb_pg1; hide nb_extra" etc)
	if ( pdest ) {
		wildcard = pdest - name;
	}
...
void Script_SetItemColor( itemDef_t *item, const char **args ) {
	...
	if ( String_Parse( args, &itemname ) && String_Parse( args, &name ) ) {
		itemDef_t *item2;
		int j;
		int count = Menu_ItemsMatchingGroup( static_cast<menuDef_t*> (item->parent), itemname );
		...
		for ( j = 0; j < count; j++ ) {
			item2 = Menu_GetMatchingItemByNumber( static_cast<menuDef_t*> (item->parent), j, itemname );
			if ( item2 != NULL ) {
				out = NULL;
				if ( Q_stricmp( name, "backcolor" ) == 0 ) {
...
//		Menu_ShowItemByName(item->parent, "page_*", qfalse);
		Menu_ShowItemByName( static_cast<menuDef_t*> (item->parent), "cover", qfalse );
		for ( i = 1; i <= NOTEBOOK_MAX_PAGES; i++ ) {
			Menu_ShowItemByName( static_cast<menuDef_t*> (item->parent), va( "page%d", i ), qfalse );
		}

		// show the visible one

		if ( curpage ) {
			Menu_ShowItemByName( static_cast<menuDef_t*> (item->parent), va( "page%d", curpage ), qtrue );
		} else {
			Menu_ShowItemByName( static_cast<menuDef_t*> (item->parent), "cover", qtrue );
		}
...
void Script_SetFocus( itemDef_t *item, const char **args ) {
	...
	if ( String_Parse( args, &name ) ) {
		focusItem = Menu_FindItemByName( static_cast<menuDef_t*> (item->parent), name );
...
void Script_AddListItem( itemDef_t *item, const char **args ) {
	...
	if ( String_Parse( args, &itemname ) && String_Parse( args, &val ) && String_Parse( args, &name ) ) {
		t = Menu_FindItemByName( static_cast<menuDef_t*> (item->parent), itemname );
		if ( t && t->special ) {
...
qboolean Item_SetFocus( itemDef_t *item, float x, float y ) {
	...
	oldFocus = Menu_ClearFocus( static_cast<menuDef_t*> (item->parent) );
...
float Item_Slider_ThumbPosition( itemDef_t *item ) {
	float value, range, x;
	editFieldDef_t *editDef = static_cast<editFieldDef_t*> (item->typeData);
...
qboolean Item_TextField_HandleKey( itemDef_t *item, int key ) {
	...
	if ( item->cvar ) {
		...
		if ( key == K_TAB || key == K_DOWNARROW || key == K_KP_DOWNARROW ) {
			newItem = Menu_SetNextCursorItem( static_cast<menuDef_t*> (item->parent) );
		...
		if ( key == K_UPARROW || key == K_KP_UPARROW ) {
			newItem = Menu_SetPrevCursorItem( static_cast<menuDef_t*> (item->parent) );
			if ( newItem && ( newItem->type == ITEM_TYPE_EDITFIELD || newItem->type == ITEM_TYPE_NUMERICFIELD ) ) {
				g_editItem = newItem;
			}
		}
...
static void Scroll_Slider_ThumbFunc( void *p ) {
	...
	scrollInfo_t *si = (scrollInfo_t*)p;
	editFieldDef_t *editDef = static_cast<editFieldDef_t*> (si->item->typeData);
...
void Controls_SetConfig( qboolean restart ) {
	...
	//	DC->setCVar("m_pitch", va("%f),-c::fabs( DC->getCVarValue( "m_pitch" ) ) );
	//else
	//	trap_Cvar_SetValue( "m_pitch", c::fabs( trap_Cvar_VariableValue( "m_pitch" ) ) );
...
void Display_HandleKey( int key, qboolean down, int x, int y ) {
	menuDef_t *menu = static_cast<menuDef_t*> (Display_CaptureItem( x, y ));
	if ( menu == NULL ) {
		menu = Menu_GetFocused();
	}
	if ( menu ) {
