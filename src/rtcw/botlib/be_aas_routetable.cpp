...
void AAS_RT_WriteRouteTable() {
	...
	// ident
	ident = rtcw::Endian::le( RTBID );
	botimport.FS_Write( &ident, sizeof( ident ), fp );

	// version
	version = rtcw::Endian::le( RTBVERSION );
...
qboolean AAS_RT_ReadRouteTable( fileHandle_t fp ) {
	...
	doswap = !rtcw::Endian::is_little ();

	// check ident
	AAS_RT_DBG_Read( &ident, sizeof( ident ), fp );
	rtcw::Endian::lei(ident);
	...
	rtcw::Endian::lei(routetable->numChildren);
	...
	if ( doswap ) {
		for ( i = 0; i < routetable->numChildren; i++, child++ ) {
			rtcw::Endian::lei(child->areanum);
			rtcw::Endian::lei(child->numParentLinks);
			rtcw::Endian::lei(child->startParentLinks);
		}
	}

	// parents
	botimport.FS_Read( &routetable->numParents, sizeof( int ), fp );
	rtcw::Endian::lei(routetable->numParents);
	...
	if ( doswap ) {
		for ( i = 0; i < routetable->numParents; i++, parent++ ) {
			rtcw::Endian::lei(parent->areanum);
			rtcw::Endian::lei(parent->numParentChildren);
			rtcw::Endian::lei(parent->startParentChildren);
			rtcw::Endian::lei(parent->numVisibleParents);
			rtcw::Endian::lei(parent->startVisibleParents);
		}
	}

	// parentChildren
	botimport.FS_Read( &routetable->numParentChildren, sizeof( int ), fp );
	rtcw::Endian::lei(routetable->numParentChildren);
	...
	rtcw::Endian::lei(routetable->numVisibleParents);
	...
	rtcw::Endian::lei(routetable->numParentLinks);
	...
	if ( doswap ) {
		for ( i = 0; i < routetable->numParentLinks; i++, plink++ ) {
			rtcw::Endian::lei(plink->childIndex);
			rtcw::Endian::lei(plink->parent);
		}
	}
