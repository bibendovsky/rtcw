...
void OglProgram::Deleter::operator()(OglProgram* ogl_program) const
{
	mem::destroy_object(ogl_program);
}
...
bool OglProgram::try_reload()
{
	...
	{
		rtcw::UniquePtr<OglProgram, Deleter> instance(create_new(glsl_dir_, base_name_));
		return instance->reload();
	}
	else if (source_type_ == source_type_c_string)
	{
		rtcw::UniquePtr<OglProgram, Deleter> instance(create_new(vertex_shader_source_, fragment_shader_source_));
		return instance->reload();
	}
	else
	{
...
OglProgram* OglProgram::create_new(const String&, const String&)
{
	std::abort();
	return NULL;
}

OglProgram* OglProgram::create_new(const char*, const char*)
{
	std::abort();
	return NULL;
}
