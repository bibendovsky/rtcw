...
void OglHdrProgram::destroy()
{
	mem::delete_object_unchecked(this);
}

bool OglHdrProgram::reload()
{
	return reload_internal();
}

void OglHdrProgram::unload()
{
	unload_internal();
}
...
{
	return mem::new_object_2<OglHdrProgram>(glsl_dir, base_name);
}

OglProgram* OglHdrProgram::create_new(const char* vertex_shader_source, const char* fragment_shader_source)
{
	return mem::new_object_2<OglHdrProgram>(vertex_shader_source, fragment_shader_source);
}
...
bool OglHdrProgram::reload_internal()
{
	...
	if (a_pos_vec2 >= max_vertex_attributes || a_tc_vec2 >= max_vertex_attributes)
	{
