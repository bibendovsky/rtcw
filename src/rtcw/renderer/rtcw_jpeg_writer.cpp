...
class MemoryStream : public jpge::output_stream
{
public:
	MemoryStream(void* buffer, jpge::uint buffer_size)
		:
	...
	virtual bool put_buf(const void* buffer, int size)
	{
		const jpge::uint buf_remaining = buffer_size_ - buffer_offset_;
	...
	int& dst_size)
{
	...
	if (is_succeed)
	{
		const bool init_result = encoder.init(&stream, width, height, 3, jpg_params);
		if (!init_result)
		{
			is_succeed = false;
	...
	if (is_succeed)
	{
		...
		const unsigned char* const src = static_cast<const unsigned char*>(src_data);
		const jpge::uint pass_count = encoder.get_total_passes();
		for (jpge::uint p = 0; p < pass_count; ++p)
		{
			for (int h = height - 1; h >= 0; --h)
			{
				const unsigned char* const src_scanline = src + (h * (width * 4));
				unsigned char* const dst_scanline = &line_buffer_[0];
				rgba_to_rgb(src_scanline, width, dst_scanline);
				const bool process_result = encoder.process_scanline(dst_scanline);
				if (!process_result)
				{
					is_succeed = false;
			...
			const bool process_result = encoder.process_scanline(NULL);
			if (!process_result)
			{
				is_succeed = false;
...
int JpegWriter::estimate_dst_size(int width, int height)
{
	if (width <= 0 || height <= 0)
	{
		return 0;
...
void JpegWriter::rgba_to_rgb(const unsigned char* src_row, int src_width, unsigned char* dst_row)
{
