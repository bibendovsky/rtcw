...
//CG_Printf("turn: %i\n", cg.time );
		tempIndex = BG_GetAnimScriptAnimation( clientNum, aistateEnum_t (cent->currentState.aiState & ~( 1 << 2 )), ( cent->pe.legs.yawing == SWING_RIGHT ? ANIM_MT_TURNRIGHT : ANIM_MT_TURNLEFT ) );
		if ( tempIndex > -1 ) {
			animIndex = tempIndex;
		}
	}
...
static void CG_PlayerPowerups( centity_t *cent ) {
	...
	if ( cent->pe.teslaDamagedTime > cg.time - 400 ) {
		trap_R_AddLightToScene( cent->lerpOrigin, 128 + 128 * c::sin( float (cg.time * cg.time) ), 0.2F, 0.6F, 1, 0 );
	}
